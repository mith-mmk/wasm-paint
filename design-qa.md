...
- Copy and content: labels are coherent in Japanese; fill and shape tools are available, while selection and text actions remain visibly disabled.
...
- Not verified: fill and shape rendering, tolerance behavior, all shape variants, filled-shape appearance, and undo/redo; general image file picker path (it shares the sample import path); native Save As picker; actual downloaded PNG file and dimensions; Arrow/Enter eyedropper sampling and transparent-pixel continuation.
