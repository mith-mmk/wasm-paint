...
use crate::line::{line_antialias, line_with_alpha};
use crate::point::*;
    ...
    while x >= -1.0 {
        ...
        if (ts <= theta && theta <= te) || (ts <= thetam && thetam <= te) {
            ...
            if bx[i] == f32::MIN {
                bx[i] = xx;
                by[i] = yy;
                point_antialias(screen, ox + xx, oy + yy, color, alpha, size);
            } else {
                line_antialias(
                    screen,
                    ox + bx[i],
