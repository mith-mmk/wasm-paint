...
impl PackedLayers {
    ...
    fn add_frame(
        ...
        match self.layers.get_mut(&label) {
            ...
            _ => {
                return Err(Box::new(crate::error::Error {
                    message: "Layer is none".to_string(),
                }))
            }
        };
        Ok(())
    ...
    fn init(
        ...
        if let Some(option) = opt {
            ...
            if let Some(background) = &option.background {
                let background_color = (background.red as u32) << 16
